...
BOOL IsXInputDevice( const GUID* pGuidProductFromDirectInput )
{
    ...
    // Connect to WMI 
    hr = pIWbemLocator->ConnectServer( ( const BSTR )L"\\\\.\\root\\cimv2", NULL, NULL, 0L, 0L, NULL, NULL, &pIWbemServices ) ;
    ...
    hr = pIWbemServices->CreateInstanceEnum( ( const BSTR )L"Win32_PNPEntity", 0, NULL, &pEnumDevices ) ;
    ...
    for(;;)
    {
        ...
        for( size_t iDevice = 0; iDevice < uReturned; ++iDevice )
        {
            ...
            hr = pDevices[ iDevice ]->Get( L"DeviceID", 0L, &var, NULL, NULL ) ;
	...
	// 情報表示
	{
		...
		if( InputSysData.PF.UseXboxControllerDirectInputFlag == FALSE )
		{
			...
			if( _WCSSTR( tszInstanceNameUPR, L"XBOX 360 FOR WINDOWS" ) != NULL ||
				...
				IsXInputDevice( &pdinst->guidProduct ) )
			{
